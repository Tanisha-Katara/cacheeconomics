"""Proposed for litellm at tests/test_litellm/test_prompt_cache_min_tokens_aliases.py

Mocked only, no network, per the contributing guide.

The regression these cover: an Anthropic model served through a reseller alias
had no `prompt_cache_min_tokens`, so `get_prompt_cache_min_tokens` fell back to
DEFAULT_MINIMUM_PROMPT_CACHE_TOKEN_COUNT (1024). For models whose real minimum
...
# This file is written to live in LiteLLM's own suite, so it imports LiteLLM.
# Collected from this repository, where LiteLLM is not a dependency, an
# unconditional import made `python3 -m pytest` fail during collection -- the
# whole suite unrunnable by the obvious command, because of a file that is not
# even part of this package.
pytest.importorskip("litellm", reason="contribution test; LiteLLM is not a "
                                      "dependency of this repository")

from litellm.constants import DEFAULT_MINIMUM_PROMPT_CACHE_TOKEN_COUNT  # noqa: E402
from litellm.utils import get_prompt_cache_min_tokens  # noqa: E402

# (alias, expected minimum). Every value already appears in the cost map on the
# canonical alias for the same model; these are the reseller entries that were
# missing it.
...
def test_minimums_are_non_monotonic_across_generations():
    ...
    If this ever starts passing by accident because everything collapsed to one
    value, the resolution is broken rather than the models having converged.
    """
...
def test_an_unknown_model_still_falls_back_rather_than_raising():
    """The fallback is correct behaviour for a model nobody has recorded; the
    bug was reaching it for models that are recorded elsewhere in the file."""
