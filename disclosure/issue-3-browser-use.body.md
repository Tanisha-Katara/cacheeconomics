...
Correction to an earlier version of this issue: threading a `ttl` through `_serialize_cache_control` alone is not sufficient. There are three places that construct `CacheControlEphemeralParam`, and two of them bypass that helper:

- `serializer.py:57` — inside `_serialize_cache_control`, the one I originally pointed at
- `serializer.py:118` — direct, on the string-content path
- `chat.py:362` — direct

So the options are either to consolidate all three through a single TTL-aware helper, or to patch each site individually. Consolidating seems better: it means the next TTL-related change only has one place to touch, and it removes the chance of a partial fix that looks complete.

The alternative remains dropping the 1h fields from the usage model entirely, since nothing can populate them today.
...
Analysed at commit `f0aa3a8bb03779c71a5aa262d389e3bfe6b77cdc`

Edited shortly after filing to correct the remediation guidance: the original version named only `_serialize_cache_control` and missed two direct constructor sites.
